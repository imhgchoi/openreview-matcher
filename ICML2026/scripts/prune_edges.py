...
if __name__ == "__main__":
    ...
    reviewer_edges = affinity_filtered.groupby('reviewer', group_keys=False).apply(lambda x: x.nlargest(K, "score"), include_groups=True).reset_index(drop=True)
    ...
    reviewer_edges_random = reviewer_affinity_filtered.groupby('reviewer', group_keys=False).apply(lambda x: x.sample(R, replace=True), include_groups=True).reset_index(drop=True)
    reviewer_edges_random = reviewer_edges_random.drop_duplicates()

    ## PAPER SIDE
    # edges from paper side
    paper_edges = affinity_filtered.groupby('paper', group_keys=False).apply(lambda x: x.nlargest(K, "score"), include_groups=True).reset_index(drop=True)
    ...
    paper_edges_random = paper_affinity_filtered.groupby('paper', group_keys=False).apply(lambda x: x.sample(R, replace=True), include_groups=True).reset_index(drop=True)
    paper_edges_random = paper_edges_random.drop_duplicates()


    # union of all edges
    bids_edges.columns = ['paper','reviewer','score']
    print(f'bid {len(bids_edges)}\nreviewer {len(reviewer_edges)}\nreviewer random {len(reviewer_edges_random)}\npaper {len(paper_edges)}\npaper random {len(paper_edges_random)}')
