...
export Q=.7 # doesn't matter if we're not using Randomized matcher
...
SUFFIX="position_rev_Feb3"
SUFFIX_NO_REV="position_Feb3"
SCORES_FILE="affinity_score_$SUFFIX.csv"
BIDS_FILE="bid_$SUFFIX.csv"
SUBMISSION_FILE="paper_$SUFFIX_NO_REV.csv"
REVIEWER_FILE="reviewer_$SUFFIX_NO_REV.csv"
NORMAL_CONFLICTS_FILE="conflict_$SUFFIX.csv"
QUALIFICATION_FILE="qualifications_$SUFFIX.csv"
UNQUALIFIED_REVIEWERS_FILE="unqualified_reviewers_$SUFFIX.csv"
...
if [ "$PREPROCESS_DATA" = "True" ]; then
	...
	python ICML2026/scripts/exclude_submissions.py \
		--final_submissions $DATA_FOLDER/position-paper-track-after-deskreject.csv \
	...
	python ICML2026/scripts/filter_reviewers.py \
		...
		--registered $DATA_FOLDER/position-paper-track-reviewers.csv
	...
	#### NOT NEEDED FOR POSITION MATCHING
...
# fi
######## NOT NEEDED FOR POSITION MATCHING
# same conflict, no joining
export AGG_CONSTRAINTS_FILE=$NORMAL_CONFLICTS_FILE
...
python ICML2026/scripts/reviewer_supply_after_matching.py \
	...
	--round2_affinity_scores $DATA_FOLDER/round2_affinity_scores.csv \
	--max_papers $MAX_PAPERS \
	--quota $DATA_FOLDER/$QUOTA_FILE \
...
if [ "$COUNTRY_CONSTRAINTS" = "True" ]; then
	...
	python ICML2026/scripts/exclude_reviewers.py \
		...
		# this was from the main matching script
		# $DATA_FOLDER/unregistered_reviewers.csv \
